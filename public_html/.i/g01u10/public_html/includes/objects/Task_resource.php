...
class Task_resource extends MysqlHandler{
    ...
    public function deleteByIds($id_resource, $id_task){
        ...
        if(!$result = $this->_adapter->query($stmt)){
            return 'Не могу удалить значения';
        }
    }

    public function deleteByTaskId($id_task){
        $stmt = 'DELETE FROM '.$this->_table.' where id_task = '. $id_task;
        if(!$result = $this->_adapter->query($stmt)){
            return 'Не могу удалить значения';
        }
    }

    public function deleteByIdentificator($identificator){
        $stmt = 'DELETE FROM '.$this->_table.' where id_resource = '. $identificator;
        if(!$result = $this->_adapter->query($stmt)){
            return 'Не могу удалить значения';
        }
    }
